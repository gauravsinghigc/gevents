...
/**
 * 
 * this Google calendar API handler class is a custom PHP libaray to handle the google calendar PAI calls.
 * 
 * @class GoogleCalendarApi
 * @author Gauravsinghigc
 * @link http://gauravsinghigc.in
 * @version 1.0
 * 
 * 
 */
...
class GoogleCalendarApi
{
  ...
  const CALENDAR_TIMEZONE_URL = "https://www.googleapis.com/calendar/v3/users/me/settings/timezone";
  ...
  public function GetAccessToken($client_id, $redirect_uri, $client_secret, $code)
  {
    ...
    if ($http_code != 200) {
      $error_msg = "ERROR: Failed to retrieve access token!";
      if (curl_errno($ch)) {
  ...
  {
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, self::CALENDAR_TIMEZONE_URL);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    ...
    if ($http_code != 200) {
      $error_msg = "ERROR: Failed to fetch Timezone information";
      if (curl_errno($ch)) {
        $error_msg = curl_error($ch);
      }
  ...
  public function GetCalendarList($access_token)
  {
    ...
    if ($http_code != 200) {
      $error_msg = "ERROR: Failed to fetch calendar list";
      if (curl_errno($ch)) {
        $error_msg = curl_error($ch);
      }
  ...
  public function CreateCalendarEvent($access_token, $calendar_id, $event_data, $all_day, $event_datetime, $event_timezone)
  {
    ...
    if ($http_code != 200) {
      $error_msg = "ERROR: Failed to create event";
      if (curl_errno($ch)) {
        $error_msg = curl_error($ch);
      }
  ...
  private function getTimezoneOffset($timezone = "Asia/Kolkata")
  {
    $current = timezone_open($timezone);
    $utcTime = new \DateTime("now", new \DateTimeZone("UTC"));
    $offsetInSecs = timezone_offset_get($current, $utcTime);
    $hourdAndSec = gmdate("H:i", abs($offsetInSecs));
    return stripos($offsetInSecs, "-") === false ? "+{$hourdAndSec}" : "-{$hourdAndSec}";
  }
}
